...
public class IsStringDecomposableIntoWords {
	...
	decomposeIntoDictionaryWords(String domain, Set<String> dictionary) {
		...
		for (int i = 0; i < domain.length(); i++) {
			String str1 = domain.substring(0, i + 1);
			...
			for (int j = 0; j < i; j++) {
				String str2 = domain.substring(j + 1, i + 1);
				if (lastLength[j] != -1 && dictionary.contains(str2)) {
					lastLength[i] = i - j;
					break;
				}
			}
	...
	public static void main(String[] args) {
		...
		Set<String> dictionary = Set.of("a", "man","plan", "canal");
		decomposeIntoDictionaryWords(domain, dictionary);
