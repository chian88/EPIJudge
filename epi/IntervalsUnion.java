...
public class IntervalsUnion {
	...
	public static List<Interval> unionOfIntervals(List<Interval> intervals) {
		...
		Collections.sort(intervals, (a, b) -> {
			int cmpLeft = Integer.compare(a.left.val, b.left.val);

			if (cmpLeft != 0) return cmpLeft;

			return a.left.isClosed && !b.left.isClosed ? -1 :
					!a.left.isClosed && b.left.isClosed ? 1 : 0;
		});
		Interval.Endpoint rightMax = intervals.get(0).right;

		for (int i = 0; i < intervals.size() - 1; i++) {
			Interval current = intervals.get(i);
			Interval next = intervals.get(i + 1);

			if (rightMax.val > next.left.val) {
				// union.
				if (next.right.val > rightMax.val) {
					rightMax = next.right;
				} else if (next.right.val == rightMax.val) {
					rightMax.isClosed = (rightMax.isClosed || next.right.isClosed);
				}
				intervals.remove(i + 1);
				i--;

			} else if (rightMax.val == next.left.val &&
					(rightMax.isClosed != next.left.isClosed ||
							rightMax.isClosed && next.left.isClosed)) {
				// also merge
				// same values, both diff is closed status, or both also closed.
				if (next.right.val > rightMax.val) {
					rightMax = next.right;
				} else if (next.right.val == rightMax.val) {
					rightMax.isClosed = (rightMax.isClosed || next.right.isClosed);
				...
				current.right = rightMax;
				rightMax = next.right;
			}
		}

		intervals.get(intervals.size() - 1).right = rightMax;
