...
public class GraphClone {
	...
	public static GraphVertex cloneGraph(GraphVertex graph) {
		...
		Map<GraphVertex, GraphVertex> memo = new HashMap<>();
		q.add(graph);
		memo.put(graph, new GraphVertex(graph.label));
		while (!q.isEmpty()) {
			GraphVertex curr = q.poll();

			for (GraphVertex neighbor : curr.edges) {
				if (!memo.containsKey(neighbor)) {
					memo.put(neighbor, new GraphVertex(neighbor.label));
					q.add(neighbor);
				}

				memo.get(curr).edges.add(memo.get(neighbor));
			}
