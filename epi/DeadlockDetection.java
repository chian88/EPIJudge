...
public class DeadlockDetection {
	...
	public static boolean isDeadlocked(List<GraphVertex> graph) {
		...
		for (GraphVertex v : graph) {
			Set<GraphVertex> path = new HashSet<>();
			boolean hasCycle = hasCycleDFS(v, path);
			if (hasCycle) return true;
		}
		return false;
	}

	static boolean hasCycleDFS(GraphVertex node, Set<GraphVertex> path) {
		...
		List<GraphVertex> neighbors = node.edges;
