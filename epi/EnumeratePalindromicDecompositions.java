...
public class EnumeratePalindromicDecompositions {
	...
	public static List<List<String>> palindromeDecompositions(String text) {
		...
		directedPalindromeDecomposition(text, 0, new ArrayList<>(), result);
		return result;
	}

	private static void directedPalindromeDecomposition(String text, int offset,
		...
		for (int i = offset + 1; i <= text.length(); i++) {
			...
			if (isPalindrome(prefix)) {
				...
				directedPalindromeDecomposition(text, i, partialPartition, result);
	...
	public static void main(String[] args) {
		System.out.println(palindromeDecompositions("2222"));
		System.exit(
				GenericTest
